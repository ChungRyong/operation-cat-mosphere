...
func _on_enemy_died(enemy: Node, _reward: int) -> void:
	pass
...
func _draw_background() -> void:
	draw_rect(Rect2(0, 0, 1280, 720), Color(0.08, 0.06, 0.12, 1.0))
	for x in range(0, 1280, 64):
		for y in range(0, 720, 64):
			if (x / 64 + y / 64) % 2 == 0:
				draw_rect(Rect2(x, y, 64, 64), Color(0.09, 0.07, 0.14, 1.0))
...
func _draw_base() -> void:
	var base_pos := Vector2(1240, _paths[0].curve.get_point_position(_paths[0].curve.point_count - 1).y if not _paths.is_empty() else 360)
