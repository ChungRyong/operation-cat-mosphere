# Operation Cat-mosphere - Sprite Generation Prompts (Nano Banana 2)

**Common Style Keywords (all prompts must include):**
`2D pixel art, top-down perspective, transparent background, game sprite asset, cute cartoon style, vibrant colors, clean outlines, sharp edges, non-blurry, pure colors, aliased, flat shading, no gradients, no anti-aliasing, lighting from top-left, consistent shadows`
...
### Phase 1
```
2D pixel art game boss, top-down view, giant red alien jelly king, large round body with golden crown on top, menacing grin, dark red translucent body, royal purple cape or collar, glowing yellow eyes, imposing and powerful, small jelly minions pattern on body, 128x128 pixel sprite, transparent background, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, game asset
```

### Phase 2 (Enraged)
```
2D pixel art game boss, top-down view, giant enraged alien jelly king, bright red-orange glowing body, golden crown tilted, furious expression, red energy aura pulsing around body, cracks of light on surface, faster and more dangerous appearance, veins of orange energy visible, 128x128 pixel sprite, transparent background, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, game asset
```

### Death
```
2D pixel art game boss death animation, top-down view, giant red alien jelly king exploding into pieces, golden crown flying off and falling, jelly fragments scattering in all directions, bright explosion effect, dramatic defeat moment, 128x128 pixel sprite, transparent background, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, game asset
```
...
### Explosion
```
2D pixel art explosion effect, top-down view, orange and yellow burst with debris particles, 3-frame animation sequence in a row, small compact explosion, 48x48 per frame, transparent background, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, game VFX asset
```

### Stun Stars
```
2D pixel art stun effect, small yellow spinning stars circling above character head, cartoon dizzy effect, 32x32 pixel sprite, transparent background, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, game VFX asset
```

### Laser Reflect
```
2D pixel art laser reflection effect, blue laser beam bouncing off mirror surface, bright white flash at reflection point, energy scatter particles, 48x48 pixel sprite, transparent background, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, game VFX asset
```

### Ultimate Shockwave
```
2D pixel art shockwave effect, top-down view, golden circular energy wave expanding outward, bright center fading to transparent edges, heroic powerful aura, 128x128 pixel sprite, transparent background, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, game VFX asset
```

---

## 6. Map Backgrounds (12 Battlefields)

> Each map needs a tileset and background. Viewport is 1280x720. Tilesets should be **64x64 px per tile**, top-down perspective. All cat-themed world.

### Map 01 - Living Room (집사 없는 거실)
```
2D pixel art top-down game tileset, cozy living room floor, wooden parquet flooring tiles, scattered cat toys, overturned sofa cushions, sunlight patches from window, warm brown and beige tones, 64x64 pixel tiles, seamlessly tileable, repeatable pattern, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, lighting from top-left, game background asset
```

### Map 02 - Wild Backyard (야생의 뒷마당)
```
2D pixel art top-down game tileset, overgrown backyard garden, green grass tiles, small pond with lily pads, tall grass bushes for hiding, scattered fallen leaves, garden fence edges, natural green and brown tones, 64x64 pixel tiles, seamlessly tileable, repeatable pattern, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, lighting from top-left, game background asset
```

### Map 03 - Horror Basement (공포의 지하실)
```
2D pixel art top-down game tileset, dark scary basement floor, cracked concrete tiles, fog and mist patches, dim yellow light bulb glow spots, cobwebs in corners, old wooden crates, dark grey and dim yellow tones, 64x64 pixel tiles, seamlessly tileable, repeatable pattern, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, lighting from top-left, game background asset
```

### Map 04 - Cat Tower Lab (캣타워 연구소)
```
2D pixel art top-down game tileset, high-tech laboratory floor, clean white tile grid with blue glowing circuit lines, holographic display panels on walls, scattered alien tech devices, cat-shaped computer terminals, white and neon blue tones, 64x64 pixel tiles, seamlessly tileable, repeatable pattern, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, lighting from top-left, game background asset
```

### Map 05 - Kitchen of Doom (마의 구역: 주방)
```
2D pixel art top-down game tileset, dangerous kitchen floor, black and white checkered tile pattern, giant spilled oil puddle hazard zones, blurry gas range in corner, scattered utensils and broken plates, warm orange kitchen light, 64x64 pixel tiles, seamlessly tileable, repeatable pattern, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, lighting from top-left, game background asset
```

### Map 06 - Cherry Blossom Playground (벚꽃 핀 놀이터)
```
2D pixel art top-down game tileset, outdoor playground with cherry blossom trees, pink petal-covered ground, sandbox area (no-build zone), colorful slide structure, swing set, soft pink and green spring colors, 64x64 pixel tiles, seamlessly tileable, repeatable pattern, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, lighting from top-left, game background asset
```

### Map 07 - Rainy Rooftop (비 내리는 지붕 위)
```
2D pixel art top-down game tileset, building rooftop in the rain, grey concrete roof tiles, rain puddles reflecting sky, TV antenna and air ducts, wet surface sheen, dark stormy sky backdrop, grey and dark blue tones, rain drop particles, 64x64 pixel tiles, seamlessly tileable, repeatable pattern, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, lighting from top-left, game background asset
```

### Map 08 - Cat Cafe (고양이 카페)
```
2D pixel art top-down game tileset, cozy cat cafe interior, warm wooden floor tiles, small round tables with cups, cat beds and cushions scattered around, bookshelf walls, trapped ally cat cages to rescue, warm pastel pink and brown tones, 64x64 pixel tiles, seamlessly tileable, repeatable pattern, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, lighting from top-left, game background asset
```

### Map 09 - Subway Station (도심 지하철역)
```
2D pixel art top-down game tileset, underground subway station platform, grey stone platform tiles, yellow safety line at edge, train tracks with periodic train hazard, screen doors, fluorescent ceiling lights, urban grey and yellow tones, 64x64 pixel tiles, seamlessly tileable, repeatable pattern, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, lighting from top-left, game background asset
```

### Map 10 - Convenience Store (대혼란의 편의점)
```
2D pixel art top-down game tileset, wrecked convenience store interior, white tile floor with scattered products, toppled shelf maze corridors, freezer section with cold mist effect, broken neon sign lights, bright fluorescent commercial lighting, white and bright color tones, 64x64 pixel tiles, seamlessly tileable, repeatable pattern, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, lighting from top-left, game background asset
```

### Map 11 - Alien Mothership (외계 모선)
```
2D pixel art top-down game tileset, alien spaceship interior, dark metallic floor with glowing green alien circuit patterns, pulsing organic alien walls, strange containment pods, alien control panels with unknown symbols, ominous green and dark purple tones, 64x64 pixel tiles, seamlessly tileable, repeatable pattern, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, lighting from top-left, game background asset
```

### Map 12 - Moon Base (최후의 보루: 달 기지)
```
2D pixel art top-down game tileset, lunar base interior with moon surface visible, white and silver clean space station tiles, large windows showing stars and earth, low gravity floating debris particles, pressurized airlock doors, blue holographic displays, white silver and deep blue space tones, 64x64 pixel tiles, seamlessly tileable, repeatable pattern, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, lighting from top-left, game background asset
```
...
### Gold Can (골드 캔 - Core Currency)
```
2D pixel art icon, shiny golden cat food can with pull-tab lid, small sparkle highlight, luxurious golden color with subtle embossed cat face logo, 32x32 pixel sprite, transparent background, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, game UI icon asset
```

### Box Scrap (상자 스크랩)
```
2D pixel art icon, crumpled brown cardboard box piece, torn edges, recycling symbol faintly visible, simple and recognizable, 32x32 pixel sprite, transparent background, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, game UI icon asset
```

### Jelly Essence (젤리 에센스)
```
2D pixel art icon, small glowing green jelly droplet in a glass vial, bright green liquid with tiny bubbles, cork stopper on top, 32x32 pixel sprite, transparent background, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, game UI icon asset
```

### Heart (HP Icon)
```
2D pixel art icon, classic red pixel heart, simple bold shape, small shine highlight, 16x16 pixel sprite, transparent background, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, game UI icon asset
```

### Churu Box (츄르 박스 - Dawn Buff Card)
```
2D pixel art icon, colorful cat treat tube (churu stick) inside a mystery gift box with question mark, exciting reward feeling, bright colorful wrapping, 48x48 pixel sprite, transparent background, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, game UI icon asset
```

### Speed Icon (배속 버튼)
```
2D pixel art icon, forward arrow speed symbol with small clock, simple and clean, white on dark background version, 24x24 pixel sprite, transparent background, sharp edges, aliased, flat shading, no gradients, no anti-aliasing, game UI icon asset
```
...
1. **Style Anchor:** Generate **Cheese Cat** first to establish the art style. Use it as a reference image for all subsequent generations
2. **Sharpness:** If output has blurry edges despite keywords, try adding `"retro 16-bit SNES style"` or `"GameBoy Advance sprite"` to force cleaner pixel rendering
3. **Iteration:** Generate 4+ variations per character, pick the best, then use img2img or inpainting to refine
4. **Post-processing:** AI output will need manual cleanup — remove artifacts, fix pixel alignment, ensure transparent background with magic wand tool
